...
FMQ.modes = {
    ...
    renderArea() {
      ...
      if (s.phase === "listen") {
        ...
        if (FMQ.isMultiDevice?.()) {
          ...
          FMQ.renderModeLikeQuick3({ heading: `Was weiß "${mainName}" wirklich?`, subtitle: "Alle außer der Person selbst tippen gleichzeitig am Handy.", heroName: "", panelClass: "theme-range", bodyHtml: `${FMQ.modes.bestFit.answerStatusHtml(expectedIds, answerMap)}` });
          return;
        }
        const pid = FMQ.getSocialResponderId();
        const responder = FMQ.getPlayerName(pid);
    ...
    renderArea() {
      ...
      if (s.phase === "listen") {
        ...
        FMQ.renderModeLikeQuick3({
          ...
          heroName: "",
          panelClass: "theme-playlist",
          bodyHtml: `<div class="bestFitStableArea">${FMQ.modes.bestFit.transportHtml()}<div class="row" style="justify-content:center; margin-top:10px;"><button id="bestFitNewSongsBtn" class="big secondary">🔄 Andere Songs ziehen</button><button id="bestFitContinueBtn" class="primary big">Weiter zur Tipp-Runde</button></div></div>`
        });
        ...
        if (FMQ.isMultiDevice?.()) {
          ...
          FMQ.renderModeLikeQuick3({
            ...
            heroName: "",
            panelClass: "theme-playlist",
            bodyHtml: `<div class="bestFitStableArea">${FMQ.modes.bestFit.transportHtml()}${FMQ.modes.bestFit.answerStatusHtml(expectedIds, answerMap)}${allDone ? FMQ.modes.bestFit.countdownHtml("Weiter zur echten Antwort in …") : ""}<div class="row" style="justify-content:center;"><button id="bestFitNewSongsBtn" class="big secondary">🔄 Andere Songs ziehen</button></div></div>`
          });
          FMQ.modes.bestFit.bindTransport(trackA, trackB);
          FMQ.$("bestFitNewSongsBtn").onclick = () => FMQ.modes.bestFit.newSongs().catch(e => FMQ.setGameDebug(e.stack || e.message));
          FMQ.renderMultiplayerPanel?.();
        ...
        FMQ.renderModeLikeQuick3({
          ...
          heroName: "",
          panelClass: "theme-playlist",
          bodyHtml: `<div class="bestFitStableArea">${FMQ.modes.bestFit.transportHtml()}${FMQ.modes.bestFit.choiceHtml("data-main-pick")}<div class="row" style="justify-content:center;"><button id="bestFitNewSongsBtn" class="big secondary">🔄 Andere Songs ziehen</button><button id="bfRevealBtn" class="big primary" disabled>Reveal</button></div></div>`
        });
        FMQ.modes.bestFit.bindTransport(trackA, trackB);
        FMQ.$("bestFitNewSongsBtn").onclick = () => FMQ.modes.bestFit.newSongs().catch(e => FMQ.setGameDebug(e.stack || e.message));
        FMQ.$("modeArea").querySelectorAll("[data-main-pick]").forEach(btn => btn.onclick = () => {
  ...
  songChallenge: {
    ...
    createDuels() {
      ...
      const prompts = FMQ.shuffle(this.promptPools().duelPrompts);
      if (players.length < 2) return [];
      const duels = [];
      const add = (a, b) => {
        const prompt = prompts[duels.length % prompts.length] || this.pickPrompt("duel");
    ...
    startAllDuelVotes(st) {
      if (st.votePromptStarted) return;
      st.votePromptStarted = true;
      FMQ.startMultiplayerPrompt?.({
        id: crypto.randomUUID(),
        type: "songChallengeDuelVote",
        kind: "multiDuelVote",
        title: "Song-Duell Voting",
        text: "Stimme alle Duelle ab. Namen werden erst nach der Punktevergabe aufgelöst.",
        voteDuelsByPlayer: this.voteDuelsByPlayer(st.duels),
        recipientIds: this.activeIds(),
        waitingText: "Warte bitte: Über deine Prompts stimmen die anderen gerade ab.",
